...
{
	Matrix3d C_q_i_w_old = q_i_w.toRotationMatrix();
	Vector3d v_i_w_old = v_i_w;
	Vector3d p_i_w_old = p_i_w;
	Quaterniond q_i_w_old = q_i_w;
	...
	q_i_w.coeffs() += (Omega*q_i_w).coeffs();
	q_i_w.normalize();

	Matrix3d C_q_i_w = q_i_w.toRotationMatrix();
...
EstimatorFull::PropagateCovariance(const Eigen::Vector3d &omega_m, const Eigen::Vector3d &a_m)
{
	...
	Matrix3d C_q_w_v = q_w_v.toRotationMatrix();
	Matrix3d C_q_i_w = q_i_w.toRotationMatrix();
	Matrix3d C_q_c_i = q_c_i.toRotationMatrix();
...
void
EstimatorFull::UpdateCamera(const Eigen::Vector3d &p_c_v, const Eigen::Quaterniond &q_c_v, const Eigen::Matrix<double,6,6> &R)
{
	Matrix3d C_q_w_v = q_w_v.toRotationMatrix();
	Matrix3d C_q_i_w = q_i_w.toRotationMatrix();
	Matrix3d C_q_c_i = q_c_i.toRotationMatrix();
	...
	Matrix<double,28,6> K = S.lu().solve(H*P).transpose();
...
EstimatorFull::ApplyCorrection( const Eigen::Matrix<double,28,1> &x_error ) 
{
	...
	q_c_i = Quaterniond( 1, x_error(19)/2.0, x_error(20)/2.0, x_error(21)/2.0 )*q_c_i; q_c_i.normalize();
	p_w_v += x_error.segment<3>(22);
	q_w_v = Quaterniond( 1, x_error(25)/2.0, x_error(26)/2.0, x_error(27)/2.0 )*q_w_v; q_w_v.normalize();
}
...
EstimatorFull::UpdateKeyframe(void)
{
	...
	q_w_v = q_i_w.conjugate() * q_c_i.conjugate();
